...
//            const auto mid_ridx = reverse_idx(mid_idx, info.width, mid_deg);
            ...
            for (auto ix=threadIdx.x, iy=threadIdx.y;
                ...
                for (int32_t lhs_deg = 1; lhs_deg < info.tile_letters; ++lhs_deg) {
                    ...
                    const auto &remainder_bound = info.levels[info.tile_letters - lhs_deg];
                    auto split = divide(xi, remainder_bound);
//                    if (xi < tile_width && yi < tile_width) {
                        lhs_val = lhs.fwd_read[get_offset(lhs_deg, split.div)];
                        rhs_val =
                            rhs.fwd_read[get_offset(rhs_deg, (split.rem * mid_stride + mid_idx) * tile_width + yi)];
...
//                    }
                ...
                for (int32_t rhs_deg = 1; rhs_deg < info.tile_letters; ++rhs_deg) {
                    ...
                    const auto &small_bound = info.levels[info.tile_letters - rhs_deg];
...
//                    if (xi < tile_width && yi < tile_width) {
                        lhs_val =
                            lhs.fwd_read[get_offset(lhs_deg, (xi * mid_stride + mid_idx) * small_bound + split.div)];
